...
console.log(header)
//tag, class 등과 같이 다수가 존재할 수 있는 대상을 DOM으로 선택하면 특정 대상 1개만 잡히는 게 아닌 Collection 기준으로 여러개가 잡히기 때문에 원하는 대상 사용 시 정확히 그 대상을 인덱스로 선택해야 한다.
...
/*
    ...
    변수 사용 시 인덱스를 붙이지 않고 변수명만 간편하게 쓸 수 있다. 대신, 인덱스가 고정되므로 다른 인덱스번호에 해당하는 태그나 클래스를 사용하고 싶을때 변수를 새로 만들어야 한다.
    console.log(aTag2);
    console.log(aTag2[1]); //aTag2[0][1] error
*/
...
//querySelector는 tag, class를 선택 시 메서드 괄호 안 문자열 대상을 기준으로 가장 먼저 인식된 대상을 출력해준다.
...
function btn_func3(){
    ...
    alert(`사용자 요청사항은 "${request}"입니다.`);
}
function btn_func4(){
    //사용자 요청사항과 주문수량을 확인하고
    //주문 버튼을 클릭하면 다음과 같이 경고창 출력하기
    //"요청사항은 oooo, 주문수량은 o개, 총 결제금액은 oooo원 입니다."
    //제품 1개당 1000원 기준
    let request = input_text.value; //요청사항 저장
    let total = Number(input_num.value); //수량 저장
    let pay = 1000; //가격
    let pay_total = total*pay;
    alert(`요청사항은 "${request}", 주문수량은 ${total}개, 총 결제금액은 ${pay_total}원 입니다.`)
}