...
//비교연산자
console.log("1"==0)
console.log("1"===1) //데이터타입 구분(문자 1과 숫자 1)
let x = "1"
let y = 1
let xy = x===y
console.log(x == y)
...
let a = 123;
let b = 0;
console.log(a && b); //0 모두 참이어야 참 반환
console.log(a || b); //123 하나만 참이어도 참 반환
let ab = (a || b) && !false; //a || b는 true, !false가 true로 전환, 둘을 &&으로 비교하면 true
console.log(ab) //true
console.log('------------------------------')
...
number_btn.addEventListener ('click', ()=>{
    if(user_number.value%2 == 0){
        alert(`당신이 입력한 ${user_number.value}은 짝수입니다`)
    }else{alert(`당신이 입력한 ${user_number.value}은 홀수입니다`)}
    /* if(user_number.value%2){
        alert(`당신이 입력한 ${user_number.value}은 홀수입니다`) 
    } -> 문구는 값이 참일때 나온다. 홀수는 나머지가 1이 남아서 true이기 때문에 문구가 출력된다.*/
})
console.log('===============================')
...
walk_btn.addEventListener('click',()=>{
    if(walk.value >= 5000){
        alert(`${walk.value}보 걸어서 목표를 달성하셨네요!`)
    }else{alert(`목표 달성까지 ${5000 - walk.value}보 부족합니다!`)}
})
console.log('===============================')
...
print_btn.addEventListener('click',()=>{
    ...
    if(result){//참에 대한 코드 블록 {}
        window.print()
        //인쇄할까요?에서 '확인'을 클릭해서 true를 반환받았을 때 if문이 실행되는 결과 만들기
    }else{//거짓에 대한 코드 블록{}}
        alert('인쇄가 취소되었습니다.')}
})