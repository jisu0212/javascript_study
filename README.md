...
### 변수 OX 퀴즈
1. `var 1num = 10;` X 숫자로 시작 불가
2. `var $num = 10;` O
3. `var 100num = 10;` X 숫자로 시작 불가
4. `var num100 = 10;` O
5. `var console = 10;` X 키워드/예약어는 사용불가
6. `var num = 10;` O
7. `var log = hello;` X 키워드/예약어는 사용불가, 문자는 따옴표 안에 작성 -> "hello"
8. `var Num = 10;` O 대문자 사용 가능하나 그럼 모두다 대문자로 시작해야 한다.
9. `var num = js;` X 문자는 따옴표 안에 작성 -> "js"
10. `var js = num;` X 키워드/예약어는 사용불가, 문자는 따옴표 안에 작성 -> "num"
## 함수(function)
* 재사용되는 문법 사용 시 함수를 이용하여 처리한다.
* 함수 내에 재사용프로그래밍 코드를 여러 줄 입력하면 다시 그복잡한 코드를 입력하지 않아도 간단하게 호출할 수 있다는 장점이 있다.
...
### `+` 연결연산자와 더하기연산자
* 좌항, 우항에 들어간 데이터 조율에 따라 연결, 더하기가 구분된다.
* 숫자 + 숫자 = 숫자 (더하기)
* "문자" + "문자" = "문자" (연결)
* "문자" + 숫자 = "문자" (연결)
### 자바스크립트 변수, 함수 주의사항
...
* `var 배열명 = new Array(5)` //빈 5개의 배열 준비(undefined 5개 준비)
* 배열 데이터는 0부터 데이터를 인식한다.
* 2번째 데이터를 출력하고 싶으면? 배열번호는 -1해야하니까 1번
* 배열의 데이터 번호 == index
* `배열명[index]`
...
* DOM.previousSibling, DOM.previousSibling.previousSibling 등..
## 자바스크립트 객체 속성
* `innerHTML` : 객체.속성
...
* 단순한 조건은 삼항연산자, 복잡하고 긴 조건은 조건문 if 사용 권장
...
* `if(조건식){참일 경우 실행결과}`
### 2단 조건문
* `if(조건식){참일 경우 실행결과}else{거짓일 경우 실행결과}`
