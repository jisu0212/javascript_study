...
// id, pw가 모두 입력 기준으로 id가 admin이 맞고 pw가 1234가 아니면 '비밀번호를 확인하세요' error 출력
// id, pw가 모두 입력 기준으로 id가 admin이고 pw도 1234면 로그인 성공 팝업 출력
//디자인 스타일 조건 추가
//입력 안해서 생긴 에러 -> 초록,700 굵기
...
// id, pw를 입력하지 않고 로그인 버튼을 클릭 시 error 위치에 "정보를 입력하세요" 출력
// 1. 클릭 이벤트 생성
let error_message = ''
login_btn.addEventListener('click',()=>{
    // 2. 조건문으로 id, pw 입력했는가? 확인하기
    if(user_id.value == "" || user_pw.value == ""){
        // 3. 위 2번이 입력 안 했다면 "정보를 입력하세요"
        //error_p.innerHTML = '정보를 입력하세요'
        if(user_id.value =='' && user_pw.value ==''){
            // 3-3. 둘다 안 적고 로그인을 눌렀다면 '아이디와 비밀번호 입력하세요' 출력
            error_message = '아이디와 비밀번호를 입력하세요'
        }else if(user_id.value ==''){
            // 3-1. 아이디를 안 적고 로그인을 눌렀다면 '아이디 입력하세요' 출력
            error_message = '아이디를 입력하세요'
        }else if(user_pw.value == ''){
            // 3-2. 비밀번호를 안 적고 로그인을 눌렀다면 '비밀번호 입력하세요' 출력
